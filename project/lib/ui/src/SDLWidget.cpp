...
void QSDLScreenWidget::resizeEvent(QResizeEvent*)
{
    ...
    //
    char variable[64];
    sprintf(variable, "SDL_WINDOWID=0x%llx", winId());
    putenv(variable);
...
#ifndef __DARWIN__
    screen = SDL_CreateWindow("Yo", 0, 0, QApplication::desktop()->width(), QApplication::desktop()->height(), SDL_SWSURFACE);
#else
    screen = SDL_CreateWindow("Yo", 24, 0, QApplication::desktop()->width(), QApplication::desktop()->height(), SDL_SWSURFACE);
#endif
    if (!screen) {
        ...
        while (!isStop && isEnd >= 0 && ++packetCount < 10) {
            ...
            if (packet.stream_index == videoStream) {
                ...
                if (frameFinished) {
                    ...
                    SDL_UpdateYUVTexture(bmp, &rect, pict.data[0], pict.linesize[0], pict.data[1], pict.linesize[1], pict.data[2], pict.linesize[2]);
                }
            }
